...
export default function TextInput(
    { name, required, sensitive, placeholder }: TextInputProps,
) {
    ...
    return (
        <div className="p-1 m-1 text-[var(--text-primary)] border-1 border-color-[var(--accent-green)] rounded-md w-full max-w-sm">
            <input name={name} type={type} required={required} placeholder={placeholder} className="appearance-none border-none outline-none bg-transparent p-0 m-0"/>
            {sensitive && (
                <button
                    type="button"
